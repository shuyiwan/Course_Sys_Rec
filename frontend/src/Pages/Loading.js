...
import '../Styles/eyeball.css';

const Loading = () => {
    return (
        <div className="animationContainer">
            {/* Eyeball Animation Elements */}
            <div className="eye"></div>
            {[...Array(14)].map((_, i) => <div key={i} className={`circle-${i + 1}`}></div>)}
            <div className="glitch"></div>
            {[...Array(3)].map((_, i) => <div key={i} className={`fragment-${i + 1}`}></div>)}
            
            {/* SVG with clipPath definitions */}
